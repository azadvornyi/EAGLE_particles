...
def get_properties(itype, property, snapshot, gn, sgn, cent, region_size, fname_read, path_to_dump):
    """
    ...
    for i in range(len(gn)):
        data_dump = {}
        mask = np.logical_and(data['GroupNumber'] == gn[i], data['SubGroupNumber'] == sgn[i])

        for att in data.keys():
            data_dump[att] = data[att][mask]
        data_dump = pd.DataFrame.from_dict(data_dump)
        print(data_dump)
        len_dict = len(data['GroupNumber'])
        
        if itype == 1:
            m_dm = read_dataset_dm_mass(fname_read)
            data_dump['Mass'] = m_dm * np.ones(len_dict, dtype='f8')

        #data_dump['ParticleType'] = np.ones(len_dict, dtype='f8') * itype
        
        path = 'table_2/' + str(path_to_dump[i])
        print(path)
        print(type(path))
        print(data_dump)
        
        d.dump(path, data_dump)
        # return data
