...
#
# snap_infall.select_region(*region)
# snap_quench.select_region(*region)
...
def get_properties(itype, property, snapshot, gn, sgn, cent, region_size, fname_read):
    ...
    :param property: list of names of the wanted attributes, should always start with (exclude 'Mass' for dm particles) ['GroupNumber','SubGroupnumber','property1',..]
...
gn_q = 11
sgn_q = 3
data_gas_infall  = get_properties(id_g, prop_gas, snap_infall, gn_inf, sgn_inf, centre_at_i_time*h_global_i, 0.5, fname_infall)
data_star_infall  = get_properties(id_s, prop_star, snap_infall, gn_inf, sgn_inf, centre_at_q_time*h_global_q, 0.5, fname_quench)

data_gas_quench  = get_properties(id_g, prop_gas, snap_quench, gn_q, sgn_q, centre_at_q_time*h_global_q, 0.5, fname_quench)
...
#
# def read_next_snapshot(first_snapshot, next_snapshot):
#     for
